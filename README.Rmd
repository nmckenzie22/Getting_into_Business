...
ggplot(dataset, aes(x = `price`)) +
  ...
  theme_minimal()
```
As with most datasets we can see that there are outliers 
...
```
Analysis. Finally we can look at a scatter plot for the sale price of the house vs the total area of square foot living space. 
